...
@section('content')
    <div class="container w-full h-screen m-auto flex items-center justify-center">
        <div
            class="form-holder relative w-[850px] sm:h-[550px] rounded-3xl shadow-[0_0_30px_rgba(0,0,0,0.2)] overflow-hidden bg-white m-5 h-[calc(100vh-20px)] {{ $errors->has('register_form') ? 'active' : '' }}">
            {{-- login form  --}}
            <div
                class="absolute login-form sm:w-1/2 sm:h-full sm:-right-0 flex items-center text-center z-10  transition-all ease-in-out duration-700 delay-[1.2s] w-full h-[70%] bottom-0">
                <form id="login-form" action="{{ route('user.login.process') }}" class="p-4 w-full" method="POST">
                    @csrf
            ...
            {{-- register form  --}}
            <div
                class="absolute register-form sm:w-1/2 sm:h-full w-full h-[70%] flex items-center sm:-right-0 text-center invisible transition-[visibility,all] duration-[0ms,700ms] delay-[1s,0s] bottom-0 z-10">
                <form id="register-form" action="{{ route('user.register') }}" class="p-4 w-full" method="POST">
                    ...
                    <div class="mb-4">
                        <div class="flex">
                            <span
                                class="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-e-0 border-gray-300 rounded-s-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                                <i
                                    class="mdi mdi-account-circle w-4 h-4 text-gray-500 dark:text-gray-400 text-[1.25rem]"></i>
                            </span>
                            <input type="text" id="website-admin"
                    ...
                    <div class="flex items-center mb-4">
                        ...
                        <label for="checkbox-1" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">I agree to
            ...
            {{-- toggle box  --}}
            <div
                class="toggle-box absolute h-full w-full before:absolute before:content-[''] sm:before:w-[300%] sm:before:h-full before:bg-blue-500 before:z-10 sm:before:-left-[250%] sm:before:rounded-[150px] before:rounded-[100px] before:transition-all before:ease-in-out before:duration-[1.8s] sm:before:top-0 before:-top-[270%] before:w-full before:h-[300%] left-0">
                <div
                    class="toggle-panel toggle-left absolute w-full sm:w-1/2 h-[30%] sm:h-full flex flex-col justify-center items-center text-white group *:mb-4 z-10 top-0">
                    <h1 class="sm:text-4xl text-2xl font-bold">Hello Welcome!</h1>
                    <p class="">Don't have an account</p>
                    <button
                        class="w-[160px] h-[46px] bg-transparent border-solid border-2 border-white shadow-none font-medium rounded-lg text-sm px-5 py-2.5 text-center register-btn">Register
                        Now</button>
                </div>
                <div
                    class="toggle-panel toggle-right absolute w-full sm:w-1/2 h-[30%] sm:h-full flex flex-col justify-center items-center text-white group *:mb-4 z-10 sm:-right-1/2 delay-700 -bottom-[30%] sm:bottom-0">
                    <h1 class="sm:text-4xl text-2xl font-bold">Welcome Back!</h1>
                    <p class="">Already have an account?</p>
                    <button
                        class="w-[160px] h-[46px] bg-transparent border-solid border-2 border-white shadow-none font-medium rounded-lg text-sm px-5 py-2.5 text-center login-btn">Login
                        Now</button>
                </div>
            </div>
        </div>
